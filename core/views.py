...
class HomeView(ListView):
    ...
    def get_context_data(self, **kwargs):
        # Get the base context from the ListView
        context = super().get_context_data(**kwargs)
        
        # Add category information to the context
        
        
        # If you want to filter items by the first category, for example:
       
        context['category_list']  = Category.objects.all()
        context['object_list_item'] = Item.objects.all()
...
class ItemDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        related_products = Item.objects.filter(category=current_item.category, is_active=True).exclude(id=current_item.id)[:4]  # Limit to 4 related products
        # context['object'] = current_item
